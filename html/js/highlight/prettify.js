...
(function() {
  ...
  var PR_KEYWORD = "kwd";
  /**
   * token style for an error
   * @const
   */
  var PR_ERROR = "err"; // (A.R)
  /**
  ...
  function combinePrefixPatterns(regexs) {
    ...
    for (var i = 0, n = regexs.length; i < n; ++i) {
      ...
      if (regex.ignoreCase) {
        ...
        /[a-z]/i.test(regex.source.replace(
  ...
  function sourceDecorator(options) {
    ...
    if (hc) {
      if (options["cStyleComments"]) {
        ...
        // #include <stdio.h>
        fallthroughStylePatterns.push(
          [PR_STRING, /^<(?:(?:(?:\.\.\/)*|\/?)(?:[\w-]+(?:\/[\w-]+)+)?[\w-]+\.h(?:h|pp|\+\+)?|[a-z]\w*)>/, null],
        );
      } else {
    ...
    if (regexLiterals) {
      ...
      fallthroughStylePatterns.push(
        ["lang-regex", RegExp("^" + REGEXP_PRECEDER_PATTERN + "(" + REGEX_LITERAL + ")")],
      );
    }
  ...
  registerLangHandler(
    createSimpleLexer([], [[PR_STRING, /^[\s\S]+/]]),
    ["regex"],
  );
  ...
  function $prettyPrint(opt_whenDone, opt_root) {
    ...
    var prettyPrintRe = /\blisp/; // Use <pre> tags with class "lisp" (A.R)
        ...
        if (
          ...
          if (!nested) {
            ...
            if (!langExtension) {
              ...
              var wrapper;
              if (
                !langExtension && (wrapper = childContentWrapper(cs))
            ...
            langExtension = "scm"; // Bluntly set to Scheme (A.R)
