...
//!
//! 이 예제는 SICP 3.5장의 핵심 개념을 보여주며, Rust의 Iterator 트레이트가
//! Scheme의 delay/force 메커니즘과 유사하게 어떻게 지연 평가(lazy evaluation)를
//! 제공하는지 보여줍니다.
//!
//! 실행 방법: cargo run --example streams_demo
...
fn main() {
    ...
    // 지연 평가의 효율성 데모
    println!("[10,000 - 11,000] 범위에서 두 번째 소수 찾기:");
    println!("  지연 평가는 답을 찾을 때까지만 계산합니다.");
    println!("  즉시 평가는 먼저 전체 소수 리스트를 구축합니다.\n");
    ...
    // 스트림을 이용한 제곱근 근사
    println!("반복적 개선을 통한 2의 제곱근 구하기:");
    let sqrt2_approx: Vec<f64> = SqrtStream::new(2.0).take(6).collect();
    for (i, approx) in sqrt2_approx.iter().enumerate() {
        println!("  반복 {}: {:.10}", i, approx);
    }
    println!("  실제 sqrt(2): {:.10}", 2.0_f64.sqrt());

    // 스트림 극한
    println!("\nstream_limit을 사용하여 근사치가 충분히 정확할 때 찾기:");
    let sqrt2_limited = stream_limit(SqrtStream::new(2.0), 0.0001);
    println!("  허용 오차 0.0001 내의 결과: {:?}", sqrt2_limited);
    ...
    // 미분 방정식 dy/dt = y, y(0) = 1 풀이
    println!("\ny(0) = 1 인 dy/dt = y 풀이:");
    println!("  해석적 해: y(t) = e^t");
    ...
    println!("몬테카를로 방법을 사용하여 π 추정하기:");
    println!("  (GCD 테스트 기반: GCD(a,b) = 1 일 확률은 π와 관련이 있음)");
    ...
    println!("핵심 통찰:");
    println!("{}", "=".repeat(60));
    println!("1. Rust의 Iterator 트레이트는 기본적으로 지연 평가를 제공합니다.");
    println!("2. 명시적인 delay/force가 필요 없습니다 - 이터레이터는 이미 지연 방식입니다.");
    println!("3. 이터레이터 콤비네이터(map, filter, take)는 자연스럽게 합성됩니다.");
    println!("4. 무한 스트림은 유한 스트림만큼이나 자연스럽습니다.");
    println!("5. 이터레이터를 사용하여 상태를 함수형으로 관리할 수 있습니다.");
    println!("6. 신호 처리 및 미분 방정식이 깔끔하게 매핑됩니다.");
    println!("{}", "=".repeat(60));
}
